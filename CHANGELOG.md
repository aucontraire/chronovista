...
- **The transcript download no longer 500s when two requested languages resolve to the same stored language code (#306).** With a base + region preference pair (e.g. `de` and `de-DE`), the region-qualified track matching can resolve both to one stored `language_code`. The endpoint then tried to persist the same `video_transcripts` primary key twice in one request; the second insert hit the key, and because the error was swallowed without a rollback, the session was poisoned and the whole request failed with a 500 (nothing saved). The download now de-duplicates by the resolved language code within a request (skipping the second), isolates each language's write in a savepoint so a stray integrity error can't poison the session, and handles the integrity error explicitly. YouTube retrieval and the successful path are unchanged.
...
### Added
- **Metadata for deleted videos can now be recovered from a third-party index.** `chronovista recover filmot` fills the title, owning channel and duration of videos YouTube no longer serves, from an index that retains records for removed videos. It is a sibling of `recover video` rather than an option on it: the two sources answer different questions and fail differently, and composing them behind one invocation would make the command the de-facto abstraction before there is a third source to generalise from. Requests are limited to one per second — the archive publishes no rate limits and returns no rate-limit headers, so the restraint is ours to keep.
- **The recovery is fill-only by construction, not by convention.** A third-party archive is weaker evidence than what the platform or your own export said, so it may fill a gap and may never contest a value. Each field's gate is re-asserted in the `UPDATE`'s own `WHERE` clause, not only when the candidate was read, so a writer that fills a field between selection and write is not overwritten — otherwise fill-only would be true by timing rather than by construction. Publication dates are never written: the value is not a parameter of the merge function at all, so no future edit can begin writing it by accident. No channel records are created; a channel the archive names but the library does not hold is reported, because creating a row to satisfy a foreign key is how placeholder data gets laundered into looking real.
- **"The archive had no record" and "we failed to ask" are counted and reported separately** throughout the run, including when it ends early. One is terminal for a video; the other means nothing was learned about it and it should be retried. Conflating them is the defect that hid 288 playlists, and the run summary keeps them apart on screen as well as in the data.
- **`chronovista playlist restore-hidden`** brings back playlists that a faulty enrichment pass marked as deleted, listing what it would restore before it does it.

### Fixed
- **A failed API fetch is no longer read as deletion.** Playlist enrichment inferred "this playlist no longer exists" from its absence in a batch response, so a request that never completed — a timeout, a transport error, a quota refusal — was indistinguishable from an authoritative "not found". A single bad pass hid the entire playlist library. The fetch layer now tracks which identifiers it genuinely could not resolve, and only identifiers the API actually answered about are eligible to be marked deleted.
- **Deletion now requires two consecutive confirmations and an owner check.** A playlist absent from one cycle records the observation; only a second cycle that also finds it absent marks it deleted. Ownership is verified before hiding anything, so a response scoped to the wrong account cannot mass-delete a library. A guard additionally refuses any single pass that would hide more than half of at least ten playlists — a proportion that no legitimate run produces.
- **Every provenance writer now goes through the repository.** The append-only tables shipped in 0.66.0 and nothing wrote to them: `record_video` and `record_channel` had zero callers while four sites went on assigning the packed column directly, so the join table was a snapshot frozen at its migration and drifting on every recovery and sync pass. The repository's own docstring claimed nothing else wrote that column, which was aspiration stated as fact.
- **A source's recorded field list accumulates instead of being replaced.** A source that filled one field on an early pass and a different field on a later pass was recorded as having written only the later one. The path is ordinary rather than rare — a run fills a title while the owning channel is still unknown, the channel is synced later, and a second run fills the channel — and it reached 23 rows before it was noticed. Note that rows predating the column being populated hold no field list at all; the fix makes future contributions accumulate but cannot reconstruct what was never recorded.
- **The image cache no longer fails on a directory that disappears mid-scan.** Before Python 3.13, `pathlib`'s `glob`/`rglob` raise `FileNotFoundError` when an entry vanishes between enumeration and the follow-up `stat`. On Docker's overlay2 that race fires readily against a cache of roughly 16,000 thumbnails across ~1,439 sharded directories, failing on a different random directory each run — the signature of a race rather than a corrupt tree. One helper now serves every scan, and guards the `stat` as well as the walk.

### Performance
- **The ASR Error Patterns page no longer appears to hang.** `GET /api/v1/corrections/batch/diff-analysis` took ~15s. Two loops counted remaining matches with `LIKE` against a `CASE` expression, and a trigram index cannot be used when `LIKE` targets one, so every iteration was a parallel sequential scan of the whole segment corpus. Both now filter the raw columns first as an index-eligible candidate super-set and keep the `CASE` predicate as an exact re-check. Measured on 1,964,881 segments: 15.9s → 2.7–3.8s, byte-identical response.
- **Remaining-match counts are no longer computed for callers that never read them.** One `COUNT` per correction pair cost 4.68s for 106 pairs against 0.13s for the grouping query preceding it, and 101 of the 106 came back zero. Two of the three callers discarded the result; they now opt out. The CLI, which displays the number, keeps it. This is not round-trip overhead — across 106 sequential awaits that was 1ms of a 4.68s loop — the queries were doing real work.
- **Per-token counting is bounded.** A short error token yields few trigrams, so the index returns a large candidate set the recheck mostly discards: for a 3-character token, 99,210 candidates to reach 3,333 rows, 96.6% discarded, with the heap block reads dominating. The same query ranged from 0.47s to 3.58s depending on what happened to be cached, and accounted for 30–65% of the request.
- **Four table walks seek by key instead of counting past rows.** `OFFSET n` makes PostgreSQL walk and discard *n* rows to reach each window, so a full walk costs O(rows²). The same defect was fixed in the entity mention scan previously; these four loops were left out of that change to keep it scoped.

### Migration
- **Two schema changes, both additive.** An index on `user_videos.video_id`, and `playlists.unavailability_first_detected` to support two-cycle deletion confirmation. The index matters more than it looks: the primary key is `(user_id, video_id)`, so a predicate on `video_id` alone cannot use it, and the playlist detail page batch-loads its watched flags with exactly that predicate — scanning the whole table to resolve at most 100 ids. Collapsing watch history to a single canonical identity sharpened this, because `user_id` now has cardinality 1 and the leading column discriminates nothing. Measured in development (988k rows): 253ms sequential scan → 2.8ms index scan.

### Technical
- **The playlist enrichment tests were rewritten.** The previous suite asserted against mocks configured by the tests themselves and passed regardless of what the enrichment code did, including while it was hiding the entire playlist library.
...
### Fixed
- **Watch history is no longer split across duplicate user identities (Feature 060, fixes #153).** The same person's data was stored under three independent conventions — the real YouTube channel id, a `takeout_user` placeholder, and `default_user` for language preferences — because identity was *derived at each write from ambient auth state* rather than resolved once and persisted. A Takeout load before OAuth wrote `takeout_user`; a later load after OAuth wrote the channel id; two parallel histories resulted. Every downstream reader of watch history had to union across identities and hope they agreed.
  - **A prior fix (PR #98) did not work and is removed here.** Its enrichment-time migration was a *skip-on-conflict* `UPDATE … WHERE video_id NOT IN (SELECT … canonical)`, not a merge. Because the placeholder was a strict subset of the real identity, every row hit the skip condition, so it migrated **zero** rows and left the placeholder behind — silently (wrapped in `try/except` → warning) with no invariant test to catch it. It ran on every enrichment for months while appearing fixed. That dead block is deleted.
  - **New `chronovista identity` CLI** — `status`, `repair [--dry-run]`, `reset [--dry-run]`. The repair is a general **merge-then-delete** (per shared video: latest `watched_at` via `GREATEST`, logical-OR of `liked`/`saved_to_playlist`, greatest `rewatch_count`; overlapping placeholder rows deleted only after their values are merged in; the remainder re-keyed). It never assumes the placeholder is a clean subset, runs in a single transaction, is idempotent, and refuses rather than guessing on an unrecognized identity configuration (e.g. two real channel ids).
  - **Safety.** Before deleting, the repair writes a JSON pre-image of **both sides** of the merge to persistent storage — the placeholder rows *and* the survivor rows it overwrites in place, since a survivor `watched_at` that loses to a newer placeholder timestamp exists nowhere else afterward. The file is written `.provisional.json` and renamed `.committed.json` only after commit, so an aborted run can't be mistaken for a completed one. Integrity invariants (distinct watched videos, distinct liked videos, sum of per-video max rewatch count) are compared before and after **inside** the transaction and any change aborts and rolls back. `--dry-run` executes the same code path in a transaction that is rolled back instead of committed, so preview and apply cannot diverge.
  - **A canonical-identity resolver** resolves the local identity once, persists it in a new `app_identities` singleton row, and is read by every writer — so a second identity can never be minted. Once persisted the identity is final and is never promoted or re-derived. Loading is never gated on authentication: a fully offline Takeout-only user persists a documented local constant, which `identity reset` can later fold into a real channel. Language preferences are re-keyed off `default_user` in the same transaction, with a primary-key collision refused rather than guessed.
  - **API** endpoints that write user-scoped data now resolve the identity via a dependency that returns **409 `IDENTITY_NOT_ESTABLISHED`** when none exists — a read request can never mint an identity as a side effect.
  - **Guards against recurrence.** A cross-table integration test asserts every identity-bearing table uses the single canonical identity (not a per-table "≤ 1 distinct" check, which would pass while two tables used *different* single identities — the exact pre-fix state). A structural test fails the build if any identity literal is reintroduced outside the resolver. Both would have caught #153 at introduction and PR #98's no-op.
  - New Alembic migration `060_add_app_identities` (purely additive). CI's integration job now runs `alembic upgrade head`. No frontend changes, no new dependencies.
...
### Fixed
- **Onboarding "Load data export" no longer stays enabled after a completed load.** The Start button is gated by a "new data available" signal computed as `export_mtime > last_loaded_at`. Two bugs kept it stuck on: (1) `export_mtime` was the newest mtime of *every* entry in the takeout dir — including a macOS `.DS_Store` that Finder re-touches, which sat newer than the last load and permanently signalled fresh data; hidden/dotfiles are now excluded from both export detection and the mtime scan. (2) `last_loaded_at` was inferred from `MAX(video.created_at)`, which never advances when a load imports a Takeout whose videos already exist (an idempotent re-import) — so the signal could never clear. A successful `load_data` run now records an explicit completion marker in the writable data dir, and `last_loaded_at` is the most recent of that marker and the newest video row. Backend-only; no schema change, no migration.

### Changed
- **Onboarding data-export card now points at the `takeout` folder, not the container path.** The card previously showed `Expected path: /data/takeout` — the path *inside* the Docker container — which was confusing since users drop their Google Takeout export into the root-level `takeout/` folder that compose bind-mounts there. The copy now leads with the `takeout` folder ("Place your Google Takeout export in the `takeout` folder…"), and the raw resolved path is demoted to a small muted `app path:` debug line. Frontend copy only.
...
- Removed 4 unused type:ignore comments in oauth_service.py
- CLI test ANSI escape code handling for CI environments
...
- **Cross-segment matches**: If a misspelling spans two adjacent transcript segments (e.g., "Katherine" at the end of segment #152 and "Shembun" at the start of segment #153), `find-replace` will not match it because each segment is searched independently. You must correct such cases manually via the inline web UI on each segment, or wait for cross-segment pattern matching support (GitHub issue #71).
- **Regex greediness**: Broad regex patterns (e.g., `Johnsm\w*`) may match unintended text in other contexts. Always use `--dry-run` first to review all matches before applying.
...
- Full ARIA combobox pattern preserved with enhanced screen reader announcements ("Peru, 910 videos, 8 variations")

**Consolidated Filter Pills (`FilterPills`, `VideoFilters`):**
- Single pill per canonical tag with variation count badge (e.g., "Peru · 8 vars")
...
- **Old bookmarks still work**: URLs with `?tag=Peru` continue to function — the backend `GET /api/v1/videos?tag=Peru` endpoint is unchanged, and the frontend reads both `tag` and `canonical_tag` URL parameters
