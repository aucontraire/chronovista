...
class TestMergeCommand:
    ...
    def test_successful_single_source_merge_exit_code_zero(self) -> None:
        ...
        with patch("chronovista.cli.tag_commands.db_manager") as mock_db, patch(
            ...
            result = runner.invoke(app, ["tags", "merge", "mejico", "--into", "peru"])
    ...
    def test_successful_merge_with_entity_hint_shows_hint(self) -> None:
        ...
        hint = "Source tag(s) had entity type(s): person. Consider classifying target 'peru'."
        mock_result = MergeResult(
            source_tags=["mejico"],
            target_tag="peru",
            aliases_moved=1,
    ...
    def test_merge_with_reason_flag_propagates_reason(self) -> None:
        ...
        with patch("chronovista.cli.tag_commands.db_manager") as mock_db, patch(
            ...
            result = runner.invoke(
                app,
                ["tags", "merge", "mejico", "--into", "peru", "--reason", "Test reason"],
            )
    ...
    def test_multi_source_merge_passes_all_sources(self) -> None:
        ...
        with patch("chronovista.cli.tag_commands.db_manager") as mock_db, patch(
            ...
            result = runner.invoke(
                app, ["tags", "merge", "mejico", "mexiko", "--into", "peru"]
            )
    ...
    def test_merge_value_error_exits_code_1(self) -> None:
        ...
        with patch("chronovista.cli.tag_commands.db_manager") as mock_db, patch(
            ...
            result = runner.invoke(app, ["tags", "merge", "nonexistent", "--into", "peru"])
    ...
    def test_merge_self_merge_exits_code_1(self) -> None:
        ...
        with patch("chronovista.cli.tag_commands.db_manager") as mock_db, patch(
            ...
            mock_service.merge.side_effect = ValueError(
                "Cannot merge tag 'peru' into itself"
            )
            mock_factory.return_value = mock_service

            result = runner.invoke(app, ["tags", "merge", "peru", "--into", "peru"])
...
class TestRenameCommand:
    ...
    def test_successful_rename_output_shows_old_and_new_form(self) -> None:
        ...
        assert "Peru" in result.output
        assert "Peru City" in result.output
        assert str(_OP_ID) in result.output
    ...
    def test_rename_missing_to_flag_exits_nonzero(self) -> None:
        ...
        result = runner.invoke(app, ["tags", "rename", "peru"])
    ...
    def test_rename_reason_too_long_exits_code_2(self) -> None:
        ...
        result = runner.invoke(
            app, ["tags", "rename", "peru", "--to", "Peru", "--reason", long_reason]
        )
...
class TestClassifyCommand:
    ...
    def test_classify_person_exit_code_zero(self) -> None:
        ...
        mock_result = ClassifyResult(
            ...
            canonical_form="Edsger Dijkstra",
            entity_type="person",
            entity_created=True,
        ...
        with patch("chronovista.cli.tag_commands.db_manager") as mock_db, patch(
            ...
            result = runner.invoke(
                app, ["tags", "classify", "edsger dijkstra", "--type", "person"]
            )
    ...
    def test_classify_person_output_shows_classify_successful(self) -> None:
        ...
        assert "Edsger Dijkstra" in result.output
        assert str(_OP_ID) in result.output
...
class TestInputNormalization:
    ...
    def test_merge_normalizes_hashtag_source(self) -> None:
        ...
        with patch("chronovista.cli.tag_commands.db_manager") as mock_db, patch(
            ...
            result = runner.invoke(
                app,
                ["tags", "merge", "#ChasFreeman", "--into", "Carl Backus"],
            )

        assert result.exit_code == 0, f"Output: {result.output}"
        call_kwargs = mock_service.merge.call_args[1]
        assert call_kwargs["source_normalized_forms"] == ["chasfreeman"]
        assert call_kwargs["target_normalized_form"] == "carl backus"

    def test_merge_normalizes_mixed_case_target(self) -> None:
        """Merge normalizes 'Carl Backus' to 'carl backus' before service call."""
        from chronovista.services.tag_management import MergeResult
    ...
    def test_merge_normalizes_multiple_sources(self) -> None:
        ...
        mock_result = MergeResult(
            source_tags=["chasfreeman", "carl backus latest"],
            target_tag="carl backus",
            aliases_moved=2,
            new_alias_count=4,
            new_video_count=20,
        ...
        with patch("chronovista.cli.tag_commands.db_manager") as mock_db, patch(
            ...
            result = runner.invoke(
                ...
                [
                    "tags", "merge",
                    "#ChasFreeman", "Carl Backus Latest",
                    "--into", "Carl Backus",
                ],
            )
    ...
    def test_split_normalizes_input(self) -> None:
        ...
        mock_result = SplitResult(
            original_tag="carl backus",
            new_tag="carl backus latest",
            new_canonical_form="carl backus latest",
            new_normalized_form="carl backus latest",
            aliases_moved=1,
    ...
    def test_normalize_strips_diacritics_and_casefolding(self) -> None:
        ...
        assert result.exit_code == 0, f"Output: {result.output}"
        call_kwargs = mock_service.merge.call_args[1]
        assert call_kwargs["source_normalized_forms"] == ["peru"]
        assert call_kwargs["target_normalized_form"] == "peru"
...
class TestClassifyDescriptionAndAutoCase:
    ...
    def test_classify_passes_description_to_service(self) -> None:
        ...
        mock_result = ClassifyResult(
            normalized_form="edith clarke",
            canonical_form="Edith Clarke",
            entity_type="person",
            entity_created=True,
            entity_alias_count=1,
            operation_id=_OP_ID,
