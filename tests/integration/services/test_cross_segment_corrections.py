...
class TestWordBoundaryRegexIntegration:
    ...
    * Segment 0 — "eml said something"    → matches  (word starts at text start)
    * Segment 1 — "kamloops is a city"     → no match (eml is a mid-word substring)
    * Segment 2 — "eml's government"      → matches  (word starts at text start,
    ...
    async def test_word_boundary_pattern_matches_eml_but_not_kamloops(
        ...
        Seed setup
        ----------
        Segment 0 — text = "eml said something"   → MATCH expected
        Segment 1 — text = "kamloops is a city"    → NO MATCH expected
        Segment 2 — text = "eml's government"     → MATCH expected
...
class TestCrossSegmentMatchBasic:
    ...
    Segment 155: "Katherine Johnsen"
    Segment 156: "Bound también siendo candidata"

    Pattern "Katherine Johnsen Bound" spans both segments.  With
    ``cross_segment=True``, the replacement "Katherine Johnson" should be
    ...
    async def test_basic_cross_segment_replacement(
        ...
        Seed setup
        ----------
        Segment 155 — text = "Katherine Johnsen"        (tail of name)
        ...
        * Segment 156 ``corrected_text`` still contains "también siendo candidata"
        ...
        result = await service.find_and_replace(
            db_session,
            pattern="Katherine Johnsen Bound",
        ...
        assert result.total_matched >= 1, (
            "Cross-segment pair 'Katherine Johnsen' + 'Bound …' must be counted as matched. "
        ...
        assert "Katherine Johnson" in corrected_a, (
...
class TestNoCrossSegmentFlag:
    ...
    async def test_cross_segment_pattern_not_found_without_flag(
        ...
        """
        Pattern "Katherine Johnsen Bound" spans two segments but is NOT matched
...
class TestRegexCrossSegment:
    ...
    async def test_regex_pattern_applied_to_cross_segment_pair(
        ...
        * Segment 20 corrected_text contains "Katherine Johnson".
        * Segment 21 corrected_text does not start with "Bound".
        ...
        assert "Katherine Johnson" in corrected_a, (
            f"Segment 20 must contain 'Katherine Johnson' after regex cross-segment "
            f"replacement. Got: {corrected_a!r}"
        )
...
class TestCaseInsensitiveCrossSegment:
    ...
    async def test_case_insensitive_cross_segment_match(
        ...
        """
        Pattern "katherine johnsen bound" (all lowercase) matches across segments
        "Katherine Johnsen" and "BOUND también" (mixed/uppercase) when
        ``case_insensitive=True``.

        Seed setup
        ----------
        Segment 30 — "Katherine Johnsen"          (mixed case)
        Segment 31 — "BOUND también"          (uppercase BOUND)

        Assertions
        ----------
        * ``result.total_matched >= 1``
        * Segment 30 corrected_text contains "Katherine Johnson".
        ...
        * Segment 31 corrected_text still contains "también".
        ...
        assert isinstance(result, BatchCorrectionResult)
        assert result.total_matched >= 1, (
            "Case-insensitive pattern 'katherine johnsen bound' must match "
            "'Katherine Johnsen' + 'BOUND …' across the segment boundary. "
            f"total_matched={result.total_matched}"
        )
        ...
        assert (
            "Katherine Johnson" in corrected_a
        ), f"Segment 30 must contain 'Katherine Johnson'. Got: {corrected_a!r}"
...
class TestEmptySegmentAfterCorrection:
    ...
    async def test_segment_not_deleted_when_correction_empties_it(
        ...
        """
        Correction "Katherine Johnsen Bound" → "Katherine Johnson" removes the
        ...
        Seed setup
        ----------
        Segment 40 — "Katherine Johnsen"
        Segment 41 — "Bound"          (only one word; will be fully consumed)

        Assertions
        ----------
        * ``result.total_matched >= 1``
        * Segment 40 ``corrected_text`` contains "Katherine Johnson".
        ...
        * The ``BatchCorrectionResult`` (or a side-channel warning) indicates
        ...
        assert isinstance(result, BatchCorrectionResult)
        assert result.total_matched >= 1, (
            "Cross-segment pair 'Katherine Johnsen' + 'Bound' must be matched. "
            f"total_matched={result.total_matched}"
        )
        ...
        assert (
            "Katherine Johnson" in corrected_a
        ), f"Segment 40 must contain 'Katherine Johnson'. Got: {corrected_a!r}"
...
class TestBasicCrossSegmentRevert:
    ...
    async def test_revert_restores_both_segments(
        ...
        """
        Apply cross-segment correction "Katherine Johnsen Bound" -> "Katherine Johnson",
        then revert by matching "Katherine Johnson" in the corrected text.
        ...
        assert (
            effective_a == "Katherine Johnsen"
        ), f"Segment A must be restored to 'Katherine Johnsen'. Got: {effective_a!r}"
        assert (
...
class TestPartnerCascadeRevert:
    ...
    async def test_partner_reverted_via_cascade(self, db_session: AsyncSession) -> None:
        """
        After cross-segment correction, segment A has "Katherine Johnson is a word"
        and segment B has " " (emptied).  Revert using pattern "Katherine Johnson"
        ...
        # batch_revert with pattern "Katherine Johnson" — only matches seg A.
...
class TestRevertOfEmptiedSegment:
    ...
    async def test_revert_restores_emptied_segment(
        ...
        """
        Correction "Katherine Johnsen Bound" -> "Katherine Johnson" where seg B
