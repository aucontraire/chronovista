...
class UserVideoSeeder(BaseSeeder):
    ...
    def __init__(self, user_video_repo: UserVideoRepository, user_id: str = "takeout_user"):
    ...
    async def seed(
            ...
            try:
                ...
                if user_video_create:
                    ...
                    if existing:
                        ...
                        await self.user_video_repo.create(session, obj_in=user_video_create)
                        result.created += 1
                ...
                if (i + 1) % 1000 == 0:
                    ...
                    logger.info(f"Processed {i + 1:,}/{len(watch_entries):,} entries "
        ...
        logger.info(f"👤 UserVideo seeding complete: {result.created} created, "
                   f"{result.updated} updated, {result.failed} failed "
                   f"in {result.duration_seconds:.1f}s")
        
        return result
    
    def _transform_entry(self, entry) -> Optional[UserVideoCreate]:
        ...
        if not video_id:
            import hashlib
            video_id = hashlib.md5((entry.title_url or "unknown").encode()).hexdigest()[:11]
