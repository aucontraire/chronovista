...
class TestRecordAsrAliasForBatchReplacement:
    ...
    async def test_alias_created_when_replacement_matches_entity_canonical_name(
        ...
        entity_mock.canonical_name = "Katherine Johnson"
        ...
        with patch(
            ...
            with patch(
                ...
                MockNorm.return_value.normalize.return_value = "katherine johnsom"
    ...
    async def test_regex_mode_extracts_distinct_matched_forms(
        ...
        seg1 = _make_segment(
            ...
            text="presidenta Katherine Johnsbom también", start_time=0.0,
        )
        seg2 = _make_segment(
            video_id="v1", segment_id=2,
            text="presidenta Katherine Johnmon equipara", start_time=1.0,
        )
        seg3 = _make_segment(
            video_id="v2", segment_id=3,
            text="presidenta Katherine Johnsbom quien", start_time=2.0,
        )
    ...
    async def test_regex_mode_single_form_calls_hook(
        ...
        seg = _make_segment(
            video_id="v1", segment_id=1,
            text="Katherine Johnsom here", start_time=0.0,
        )
    ...
    async def test_alias_normalized_via_tag_normalization_service(
        ...
        entity_mock = MagicMock()
        entity_mock.id = uuid.uuid4()
        entity_mock.canonical_name = "Oscar Lindgren"
        ...
        with patch(
            ...
            with patch(
                ...
                MockNorm.return_value.normalize.return_value = "oscar lindgren typo"

                await service._record_asr_alias_for_batch_replacement(
                    session,
                    pattern="Oscar Lindgren Typo",
                    replacement="Oscar Lindgren",
                    total_applied=2,
                )
            ...
            assert alias_create.alias_name_normalized == "oscar lindgren typo"
    ...
    async def test_find_and_replace_calls_hook_on_success(
        ...
        seg = _make_segment(
            video_id="v1", segment_id=1, text="Katherine Johnsom", start_time=0.0,
        )
...
class TestCreateEntityMentionsForSegment:
    ...
    async def test_exclusion_filters_some_but_not_all(
        ...
        entity = _make_entity(exclusion_patterns=["Peru City"])
        seg = _make_segment(video_id="vid1_test_x", segment_id=5)
        correction_id = uuid.uuid4()
        ...
        with patch(
            ...
            # Text has "Peru" twice: once in "Peru City" (excluded) and once standalone
            await service._create_entity_mentions_for_segment(
                mock_session,
                entity=entity,
                segment=seg,
                corrected_text="visited Peru City and then Peru",
                replacement="Peru",
                correction_id=correction_id,
            )
            ...
            mock_repo_instance.bulk_create_with_conflict_skip.assert_called_once()
            mentions = mock_repo_instance.bulk_create_with_conflict_skip.call_args[0][1]
            # First "Peru" at 8-14 overlaps with "Peru City" at 8-19 → excluded
            # Second "Peru" at 29-35 → included
            assert len(mentions) == 1
...
class TestMinimalTokenAliasRegistration:
    ...
    async def test_sub_token_registered_when_entity_matched(self) -> None:
        ...
        with patch(
            ...
            new=AsyncMock(return_value=(entity_id, "Ada Lovelace")),
        ), patch(
            "chronovista.services.asr_alias_registry.TagNormalizationService"
            ...
            await register_asr_alias(
                mock_session,
                original_text="Ada Lovelase",
                corrected_text="Ada Lovelace",
            )
