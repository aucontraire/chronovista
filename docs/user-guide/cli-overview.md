...
```bash
# Single merge
chronovista tags merge mejico --into peru
...
chronovista tags classify "ada lovelace" --type person --description "Linguist and political commentator" --reason "MIT linguistics professor"
...
- `NAME` - Canonical name for the entity (e.g., "Ada Lovelace")
...
chronovista entities create "Alan Turing" --type person --description "NSA whistleblower" --alias "Ed Snowden" --alias "Snowden"
...
chronovista entities add-alias "Renée Dubois" --alias "Renée Dubois Laurent" --alias "Jairo Calixtro"
```
