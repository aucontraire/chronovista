...
const PHONETIC_MATCH: PhoneticMatch = {
  original_text: "Barbara Jordan Reed",
  proposed_correction: "Barbara Jordan-Reed",
  confidence: 0.92,
...
describe("fetchPhoneticMatches", () => {
  ...
  it("unwraps the data array from the response envelope", async () => {
    const matches: PhoneticMatch[] = [
      PHONETIC_MATCH,
      { ...PHONETIC_MATCH, original_text: "Nora Bennett", segment_id: 99 },
    ];
    ...
    expect(result[0]?.original_text).toBe("Barbara Jordan Reed");
    expect(result[1]?.original_text).toBe("Nora Bennett");
  });
