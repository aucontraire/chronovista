...
describe('TagAutocomplete - Canonical Tags', () => {
  ...
  describe('Two-line dropdown items (FR-001)', () => {
    it('renders canonical_form and video_count on the first line', async () => {
      mockCanonicalTags({
        tags: [
          { canonical_form: 'Peru', normalized_form: 'peru', alias_count: 9, video_count: 910 },
        ],
      });
      ...
      const option = screen.getByRole('option', { name: /Peru/i });
  ...
  describe('Tag selection (T009)', () => {
    ...
    it('calls onTagRemove with normalizedForm when removing a selected tag', async () => {
      ...
      const selectedTags: SelectedCanonicalTag[] = [
        { canonical_form: 'Peru', normalized_form: 'peru', alias_count: 9 },
      ];
      ...
      const removeButton = screen.getByRole('button', { name: /remove tag Peru/i });
      await userEvent.click(removeButton);

      expect(mockOnTagRemove).toHaveBeenCalledWith('peru');
    });
  });
  ...
  describe('Fuzzy suggestions (FR-019/FR-020/FR-023)', () => {
    it('renders fuzzy suggestions in a role="group" when tags empty', async () => {
      mockCanonicalTags({
        tags: [],
        suggestions: [
          { canonical_form: 'Peru', normalized_form: 'peru' },
    ...
    it('renders fuzzy suggestion buttons with correct aria-labels', async () => {
      ...
      await waitFor(() => {
        const btn = screen.getByRole('button', { name: 'Did you mean Peru?' });
    ...
    it('clicking a fuzzy suggestion calls onTagSelect with SelectedCanonicalTag', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByRole('button', { name: 'Did you mean Peru?' })).toBeInTheDocument();
      });

      const suggestionBtn = screen.getByRole('button', { name: 'Did you mean Peru?' });
  ...
  describe('Option aria-labels for screen readers (T012 / FR-025)', () => {
    it('includes video_count and variation count in option aria-label when alias_count > 1', async () => {
      ...
      // aria-label should be "Peru, 910 videos, 8 variations"
      const option = screen.getByRole('option', { name: /Peru, 910 videos, 8 variations/i });
      expect(option).toBeInTheDocument();
    });
