...
_VALID_NORMALIZED_FORM = "edsger dijkstra"
...
def _make_classify_result(
    *,
    ...
    canonical_form: str = "Edsger Dijkstra",
...
def _make_named_entity_row(
    *,
    entity_id: uuid.UUID | None = None,
    canonical_name: str = "Edsger Dijkstra",
    entity_type: str = "person",
...
class TestClassifyTagHappyPath:
    ...
    async def test_response_body_values_match_service_and_db(self) -> None:
        ...
        assert body["canonical_name"] == "Edsger Dijkstra"
...
_DUPLICATE_NAME = "Radia Perlman"
_DUPLICATE_TYPE = "person"
_DUPLICATE_NORMALIZED = "radia perlman"


def _make_named_entity_for_duplicate(
    *,
    entity_id: uuid.UUID | None = None,
    canonical_name: str = "Radia Perlman",
    entity_type: str = "person",
...
class TestCheckDuplicateEndpoint:
    ...
    async def test_200_duplicate_found(self) -> None:
        ...
        assert existing["canonical_name"] == "Radia Perlman"
    ...
    async def test_normalization_aware_matching(self) -> None:
        """Lowercase input "radia perlman" matches entity "Radia Perlman".
        ...
        async for client in _build_client(mock_session):
            with patch(
                ...
                # Simulates normalization stripping case: "radia perlman" → "radia perlman"
                mock_normalizer.normalize.return_value = "radia perlman"

                response = await client.get(
                    _CHECK_DUPLICATE_ENDPOINT,
                    params={"name": "radia perlman", "type": "person"},
                )
        ...
        assert body["existing_entity"]["canonical_name"] == "Radia Perlman", (
            f"Expected canonical_name='Radia Perlman' in: {body['existing_entity']}"
        )
    ...
    async def test_different_type_not_duplicate(self) -> None:
        """Entity "Radia Perlman" as "person" exists, but checking with type
...
_VALID_CREATE_NAME = "alan turing"
...
def _make_create_entity_db_row(
    *,
    entity_id: uuid.UUID | None = None,
    canonical_name: str = "Alan Turing",
    entity_type: str = "person",
    description: str | None = None,
) -> MagicMock:
...
class TestCreateEntityEndpoint:
    ...
    async def test_201_success_with_correct_alias_count(self) -> None:
        """Create "alan turing" with type "person" and two user aliases.
        ...
        assert response.status_code == 201, response.text
        body = response.json()
        assert body["canonical_name"] == "Alan Turing", (
            f"Expected 'Alan Turing', got: {body['canonical_name']}"
        )
    ...
    async def test_auto_title_casing(self) -> None:
        """Lowercase input "radia perlman" produces canonical_name="Radia Perlman".
        ...
        assert response.status_code == 201, response.text
        body = response.json()
        assert body["canonical_name"] == "Radia Perlman", (
            f"Expected auto-title-cased 'Radia Perlman', got: {body['canonical_name']}"
        )
    ...
    async def test_409_duplicate(self) -> None:
        ...
        existing_row.canonical_name = "Alan Turing"
        ...
        async for client in _build_client(mock_session):
            with patch(
                ...
                mock_normalizer.normalize.return_value = "alan turing"
    ...
    async def test_409_duplicate_response_body_shape(self) -> None:
        ...
        assert "alan turing" in body["detail"], (
