...
from src.chronovista.services.seeding import SeedResult, ProgressCallback
from src.chronovista.models.takeout.takeout_data import TakeoutData, TakeoutSubscription, TakeoutWatchEntry, TakeoutPlaylist, TakeoutPlaylistItem
...
class TestTakeoutSeedingService:
        ...
        return TakeoutData(
            ...
            playlists=[
                TakeoutPlaylist(
                    ...
                    videos=[
                        TakeoutPlaylistItem(video_id="abc123")
                    ]
                )
            ]
        )
    ...
    def test_initialization(self, seeding_service):
        ...
        assert seeding_service.get_available_types() == {"channels", "videos", "user_videos", "playlists"}
    ...
    def test_get_available_types(self, seeding_service):
        ...
        expected_types = {"channels", "videos", "user_videos", "playlists"}
        assert available_types == expected_types

    async def test_seed_database_all_types(self, seeding_service, mock_takeout_data, mock_session):
        ...
        mock_results = {
            ...
            "user_videos": SeedResult(created=1, updated=0, failed=0, duration_seconds=1.0),
            "playlists": SeedResult(created=1, updated=0, failed=0, duration_seconds=1.0)
        }
        
        with patch.object(seeding_service.orchestrator, 'seed', return_value=mock_results) as mock_seed:
            results = await seeding_service.seed_database(mock_session, mock_takeout_data)
            ...
            assert args[2] == {"channels", "videos", "user_videos", "playlists"}
    ...
    async def test_seed_database_specific_types(self, seeding_service, mock_takeout_data, mock_session):
        """Test seeding specific data types only."""
        mock_results = {
            "channels": SeedResult(created=5, updated=10, failed=0, duration_seconds=2.5)
        }
    ...
    async def test_seed_database_with_skip_types(self, seeding_service, mock_takeout_data, mock_session):
        ...
        with patch.object(seeding_service.orchestrator, 'seed', return_value=mock_results) as mock_seed:
            results = await seeding_service.seed_database(
                mock_session, 
                mock_takeout_data, 
                skip_types={"user_videos", "playlists"}
            )
            
            # Verify orchestrator was called with correct filtered types
            args, kwargs = mock_seed.call_args
            assert args[2] == {"channels", "videos"}

    async def test_seed_database_with_progress_callback(self, seeding_service, mock_takeout_data, mock_session):
        ...
        with patch.object(seeding_service.orchestrator, 'seed', return_value=mock_results) as mock_seed:
            ...
            args, kwargs = mock_seed.call_args
            assert len(args) == 4  # session, takeout_data, types_to_process, progress_callback
            assert args[3] == progress

    async def test_seed_incrementally(self, seeding_service, mock_takeout_data, mock_session):
    ...
    async def test_empty_data_types_handling(self, seeding_service, mock_takeout_data, mock_session):
        """Test handling when no data types are selected."""
        # Test with data_types that don't intersect with available types
        results = await seeding_service.seed_database(
            mock_session, 
            mock_takeout_data, 
            data_types={"nonexistent_type"}
        )
        
        # Should return empty results
        assert results == {}

    async def test_error_handling_in_orchestrator(self, seeding_service, mock_takeout_data, mock_session):
        """Test error handling when orchestrator fails."""
        with patch.object(seeding_service.orchestrator, 'seed', side_effect=Exception("Test error")):
    ...
    async def test_conflicting_data_types_and_skip_types(self, seeding_service, mock_takeout_data, mock_session):
