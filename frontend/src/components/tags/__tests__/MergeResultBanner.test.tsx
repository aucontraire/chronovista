...
describe("MergeResultBanner", () => {
  ...
  it("renders a success summary naming the sources and target", () => {
    ...
    expect(screen.getByText(/professor grace hopper/i)).toBeInTheDocument();
    expect(screen.getAllByText(/grace hopper/i).length).toBeGreaterThan(0);
  });
  ...
  describe("entity hint (FR-016)", () => {
    it("surfaces the entity_hint when present", () => {
      render(
        <MergeResultBanner
          result={makeResult({ entity_hint: "Linked to entity: Grace Hopper (person)" })}
        />
      );
      expect(
        screen.getByText("Linked to entity: Grace Hopper (person)")
      ).toBeInTheDocument();
    });
