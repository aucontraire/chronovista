...
const hannahFryTag = {
  canonical_form: "Grace Hopper",
  normalized_form: "grace hopper",
  alias_count: 5,
  video_count: 30,
};
...
describe("TagMergeSelector", () => {
  ...
  describe("search results", () => {
    it("renders result rows with Add as source and Set target actions", async () => {
      ...
      expect(
        screen.getByRole("button", { name: /add professor grace hopper as a source tag/i })
      ).toBeInTheDocument();
      expect(
        screen.getByRole("button", { name: /set professor grace hopper as the merge target/i })
      ).toBeInTheDocument();
    });
  ...
  describe("validation", () => {
    it("prevents adding the current target as a source and shows an error (FR-011)", async () => {
      ...
      const addButton = screen.getByRole("button", {
        name: /add grace hopper as a source tag/i,
      });
    ...
    it("prevents setting an existing source as the target and shows an error (FR-011)", async () => {
      ...
      const setTargetButton = screen.getByRole("button", {
        name: /set professor grace hopper as the merge target/i,
      });
    ...
    it("disables 'Add as source' for a tag already selected as a source (FR-012)", async () => {
      ...
      const addButton = screen.getByRole("button", {
        name: /add professor grace hopper as a source tag/i,
      });
      expect(addButton).toBeDisabled();
  ...
  describe("selected tag pills", () => {
    it("renders the target pill and a remove control", async () => {
      ...
      const removeButton = screen.getByRole("button", {
        name: /remove grace hopper as target/i,
      });
    ...
    it("renders source pills with individual remove controls", async () => {
      ...
      const removeButton = screen.getByRole("button", {
        name: /remove professor grace hopper as source/i,
      });
      await userEvent.click(removeButton);
      expect(onRemoveSource).toHaveBeenCalledWith("professor grace hopper");
    });
