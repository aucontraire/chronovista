...
class TestStandaloneCreationEndToEnd:
    ...
    async def cleanup_snowden(
        ...
        """Remove any leftover Alan Turing entity rows before and after each test."""
    ...
    async def test_standalone_creation_end_to_end(
        ...
        async with integration_session_factory() as session:
            ...
            assert db_entity.canonical_name == "Alan Turing", (
            ...
            assert "Alan Turing" in alias_names, (
...
class TestDuplicateDetection:
    ...
    _GARLAND_CANONICAL = "Radia Perlman"
    ...
    async def seed_garland_entity(
        ...
        """Seed a 'Radia Perlman' named entity for duplicate detection tests.

        Yields a dict with:
        - ``entity_id``: UUID of the seeded entity
        - ``canonical_name``: "Radia Perlman"
        - ``normalized_name``: "radia perlman"
    ...
    async def test_duplicate_detection_returns_correct_entity(
        ...
        Sends the query with lower-case input ("radia perlman") to verify the
...
class TestConflictOnDuplicateCreation:
    ...
    async def seed_garland_for_conflict(
        ...
        """Seed a 'Radia Perlman' named entity for the 409 conflict test.
    ...
    async def test_409_on_duplicate_creation(
        ...
        Attempts to create another "Radia Perlman" (person) while one is
    ...
    async def test_same_name_different_type_is_allowed(
        ...
        'Radia Perlman' as 'organization' should not conflict with the
        seeded 'Radia Perlman' of type 'person'.
        """
        async with integration_session_factory() as session:
            # Pre-clean any existing 'radia perlman' of type 'organization'
            existing = (
                await session.execute(
                    select(NamedEntityDB).where(
