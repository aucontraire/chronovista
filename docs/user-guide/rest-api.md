...
!!! tip "Canonical Tag vs Raw Tag Filtering"
    The `tag` parameter matches exact raw tag strings (case-sensitive). The `canonical_tag` parameter matches normalized tag groups — e.g., `canonical_tag=peru` returns videos tagged with "peru", "Peru", "méxico", "México", "#peru", etc. Multiple `canonical_tag` values use AND semantics (videos must match all specified groups).
...
Canonical tags group raw tag variations (case, accents, hashtags) into unified concepts. For example, the canonical tag "peru" groups "peru", "Peru", "méxico", "México", "#peru", and more. Built by the tag normalization pipeline (ADR-003).
...
!!! note "Match mode"
    `prefix` (the default) preserves the video filter's behavior — searching "Grace Hopper" finds only tags that start with it. `contains` finds a substring anywhere, so "Grace Hopper" also matches "Professor Grace Hopper". The web tag-merge UI uses `contains`.
...
```json
{
  "data": [
    {
      "canonical_form": "Peru",
      "normalized_form": "peru",
      "alias_count": 9,
      "video_count": 910
    },
    {
      "canonical_form": "Peru City",
      "normalized_form": "peru city",
      "alias_count": 4,
      "video_count": 64
    }
  ],
...
```json
{
  "data": {
    ...
    "video_count": 910,
    "top_aliases": [
      { "raw_form": "peru", "occurrence_count": 651 },
      { "raw_form": "Peru", "occurrence_count": 176 },
      { "raw_form": "méxico", "occurrence_count": 109 },
      { "raw_form": "México", "occurrence_count": 76 },
      { "raw_form": "#peru", "occurrence_count": 8 }
    ],
...
```json
{
  "source_normalized_forms": ["professor grace hopper"],
  "target_normalized_form": "grace hopper"
}
```
...
{
  "data": {
    "source_tags": ["Professor Grace Hopper"],
    "target_tag": "Grace Hopper",
    ...
    "matches": [
      {
        ...
        "language_code": "es",
        "start_time": 245.6,
        "current_text": "el presidente eml dijo",
        "proposed_text": "el presidente EML dijo",
...
```json
{
  ...
  "correction_note": "Fixed ASR misrecognition of EML",
  "auto_rebuild": true
}
```
...
```json
{
  "data": [
    {
      "error_token": "Redacted",
      "canonical_form": "Sheinbaum",
      "frequency": 14,
      "remaining_matches": 23,
      "entity_id": "01936c8a-...",
      "entity_name": "Katherine Johnson"
    }
  ]
}
```
...
```json
{
  "data": [
    {
      "segment_n_id": 934,
      "segment_n_text": "...una entrevista, Katherine Johnsen",
      "segment_n1_id": 935,
      "segment_n1_text": "Bound también siendo candidata...",
      "proposed_correction": "Sheinbaum",
      "source_pattern": "Johnsen Bound",
      "confidence": 0.85,
...
```json
{
  "data": [
    {
      "entity_id": "01936c8a-...",
      "canonical_name": "Émile Moreau Laurent",
      "entity_type": "person",
      "description": "Former President of Peru (2018–2024)",
      "status": "active",
      "mention_count": 342,
      "video_count": 87
    }
...
```json
{
  "data": {
    ...
    "video_count": 87,
    "aliases": [
      { "alias_name": "EML", "alias_type": "abbreviation", "occurrence_count": 156 },
...
```json
{
  "data": [
    {
      "video_id": "abc123",
      "video_title": "Interview with EML",
      "channel_name": "News Channel",
      "mention_count": 12,
      "mentions": [
        { "segment_id": 456, "start_time": 123.5, "mention_text": "EML" }
      ],
  ...
  "data": [
    {
      ...
      "video_id": "JIMXfrMtHas",
      "segment_id": 934,
      "video_title": "Interview with Redacted"
    }
  ]
}
```
...
```bash
# Simple tag
curl http://localhost:8000/api/v1/canonical-tags/peru

# Tag with spaces (URL-encoded)
curl "http://localhost:8000/api/v1/canonical-tags/new%20york"

# Limit aliases returned
curl "http://localhost:8000/api/v1/canonical-tags/peru?alias_limit=3"
```

#### Get Canonical Tag Videos

```bash
curl http://localhost:8000/api/v1/canonical-tags/peru/videos
```
...
curl "http://localhost:8000/api/v1/videos?canonical_tag=peru"

# Multiple canonical tags (AND semantics)
curl "http://localhost:8000/api/v1/videos?canonical_tag=peru&canonical_tag=travel"
