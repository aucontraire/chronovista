...
class TestUndoClassify:
    ...
    async def test_undo_classify_new_entity_deleted(
        ...
        assert isinstance(result, str)
        assert "edsger dijkstra" in result
...
class TestClassifyDescriptionAndAutoCase:
    ...
    def _setup_classify_mocks(
        ...
        mock_session: AsyncMock,
        *,
        tag_canonical_form: str = "Edith Clarke",
        tag_normalized_form: str = "edith clarke",
    ...
    async def test_classify_auto_case_title_cases_canonical_form(
        ...
        Expected behaviour:
        - ``tag.canonical_form`` is mutated to ``"Edith Clarke"``.
        ...
        tag, _ = self._setup_classify_mocks(
            ...
            mock_operation_log_repo,
            mock_session,
            tag_canonical_form="edith clarke",
            tag_normalized_form="edith clarke",
        )
        ...
        assert (
            tag.canonical_form == "Edith Clarke"
    ...
    async def test_classify_auto_case_false_preserves_canonical_form(
        ...
        Expected behaviour:
        - ``tag.canonical_form`` remains ``"edith clarke"`` (unchanged).
        """
        from chronovista.models.enums import EntityType
    ...
    async def test_classify_auto_case_skips_already_title_cased(
        ...
        Expected behaviour:
        - ``tag.canonical_form`` remains ``"Edith Clarke"`` (no change).
...
class TestClassifyLinkEntity:
    ...
    async def test_classify_link_entity_links_existing_entity(
        ...
        # The existing entity (e.g. "Peter Naur") that the tag should link to
        target_entity = _make_named_entity(
            normalized_form="peter naur",
            entity_type="person",
            discovery_method="user_created",
        )
        target_entity.status = "active"
        target_entity.canonical_name = "Peter Naur"
...
class TestMergeAliasUpdateColumns:
    ...
    async def test_merge_alias_update_sets_both_columns(
        ...
        target = _make_canonical_tag(
            normalized_form="grace hopper", canonical_form="Grace Hopper"
        )
        target.entity_type = None
        source = _make_canonical_tag(
            normalized_form="professor grace hopper",
            canonical_form="Professor Grace Hopper",
        )
        source.entity_type = None
        ...
        await service.merge(mock_session, ["professor grace hopper"], "grace hopper")
...
class TestMergePreview:
    ...
    async def test_preview_empty_sources_raises(
        ...
        with pytest.raises(ValueError, match="At least one source"):
            await service.merge_preview(mock_session, [], "grace hopper")
