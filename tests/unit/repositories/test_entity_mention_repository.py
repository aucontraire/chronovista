...
def _make_entity_mention_db(
    *,
    ...
    mention_text: str = "Edsger Dijkstra",
...
class TestDeleteByScope:
    ...
    async def test_delete_returns_zero_when_no_rows_match(
        ...
        count = await repository.delete_by_scope(mock_session, entity_ids=[_uuid()])
...
class TestDeleteByCorrectionIds:
    ...
    async def test_deletes_by_correction_ids(
        self, repository: EntityMentionRepository
    ) -> None:
...
class TestGetEntityIdsByCorrectionIds:
    ...
    async def test_returns_distinct_entity_ids(
        self, repository: EntityMentionRepository
    ) -> None:
    ...
    async def test_empty_correction_ids_returns_empty(
        self, repository: EntityMentionRepository
    ) -> None:
    ...
    async def test_no_matching_mentions_returns_empty(
        self, repository: EntityMentionRepository
    ) -> None:
        ...
        result = await repository.get_entity_ids_by_correction_ids(
            session, [uuid.uuid4()]
        )
...
class TestUpdateEntityCounters:
    ...
    async def test_sql_excludes_asr_error_alias_type(
        ...
        assert "asr_error" in sql_str, (
            "Expected 'asr_error' exclusion in counter SQL; " f"got: {sql_str[:500]}"
        )
    ...
    async def test_second_update_zeros_entities_with_no_visible_mentions(
        ...
        assert "mention_count" in sql_str, (
            "Zero-out UPDATE must include mention_count; " f"got: {sql_str[:400]}"
        )
        assert "video_count" in sql_str, (
            "Zero-out UPDATE must include video_count; " f"got: {sql_str[:400]}"
        )
    ...
    async def test_visible_names_join_uses_lower_for_case_insensitive_match(
        ...
        Mentions stored as 'edsger dijkstra' must match canonical name 'Edsger Dijkstra'
...
class TestGetEntityVideoIds:
    ...
    async def test_return_type_is_always_a_set(
        ...
        entity_id = _uuid()
        mock_session.execute.return_value = self._make_scalars_result(["dQw4w9WgXcQ"])
...
class TestSearchEntities:
    ...
    def _make_entity_row(
        ...
        *,
        entity_id: uuid.UUID | None = None,
        canonical_name: str = "Barbara Liskov",
    ...
    async def test_search_canonical_name_prefix_match(
        ...
        Query "Joan" should match the entity with canonical_name "Barbara Liskov".
        ...
        assert len(results) == 1
        item = results[0]
        assert item["canonical_name"] == "Barbara Liskov"
    ...
    async def test_search_empty_results(
        ...
        results = await repository.search_entities(mock_session, query="zzznomatch")
...
class TestGetEntityVideoListMultiSource:
    ...
    async def test_manual_only_video(
        ...
        mock_session.execute.side_effect = [
            self._make_transcript_vid_result(
                ["vid001"]
            ),  # manual mentions included in transcript_vid
    ...
    async def test_both_sources(
        ...
        mock_session.execute.side_effect = [
            ...
            self._make_preview_result(
                [{"segment_id": 1, "start_time": 5.0, "mention_text": "Edsger"}]
            ),
        ]
...
class TestDeleteManualAssociation:
    ...
    def _make_manual_mention(
        ...
        *,
        ...
        mention.confidence = None
        mention.detection_method = "manual"
        mention.mention_text = "Barbara Liskov"
        mention.created_at = datetime(2024, 1, 15, 10, 30, 0, tzinfo=UTC)
        return mention
    ...
    async def test_delete_successful_calls_session_delete(
        ...
        entity_id = _uuid()
        video_id = "dQw4w9WgXcQ"
        manual_mention = self._make_manual_mention(
        ...
        with patch.object(repository, "update_entity_counters", new=AsyncMock()):
    ...
    async def test_delete_counters_called_after_delete(
        ...
        with patch.object(
            repository,
            "update_entity_counters",
            new=AsyncMock(side_effect=_track_counters),
        ):
            await repository.delete_manual_association(
                mock_session, video_id=video_id, entity_id=entity_id
            )
...
class TestEntityMentionRepositorySourceMapping:
    ...
    async def test_t016_entity_in_title_and_transcript_shows_both_sources(
        ...
        video_row.video_title = "Ada Lovelace Interview"
        ...
        preview_row.mention_text = "Ada Lovelace"
        preview_result = MagicMock()
        preview_result.all.return_value = [preview_row]
