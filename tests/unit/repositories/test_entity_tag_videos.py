...
class TestGetAliasMatchedTagVideoIds:
    ...
    async def test_asr_error_alias_excluded_from_tag_matching(
        ...
        ASR error aliases (e.g., 'Emile', 'edsger') are transcript-specific
    ...
    async def test_asr_error_alias_with_valid_normalized_form_still_excluded(
        ...
        Even when an asr_error alias (e.g., 'edsger') would produce a valid
        ...
        patterns like 'Emile', 'Moreau', 'edsger' matched real YouTube tags.
        """
        entity_id = _uuid()
        expected_ids = ["vid_real_001"]

        # Call 1 returns only the non-ASR alias (DB filtered out 'edsger')
        # Call 2 returns video IDs for the valid alias
        mock_session.execute.side_effect = [
            _scalars_result(["Edsger Dijkstra"]),  # Only the name_variant alias
            _scalars_result(expected_ids),
        ]
        ...
        mock_normalizer.normalize.assert_called_once_with("Edsger Dijkstra")
