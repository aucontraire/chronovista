...
async def seed_entity_data(
    ...
    - One NamedEntity (``"Edsger Dijkstra"``, type ``"person"``).
    ...
    async with integration_session_factory() as session:
        ...
        if not existing_transcript1:
            transcript1 = VideoTranscriptDB(
                ...
                transcript_text="Edsger Dijkstra founded SpaceX. Later Edsger Dijkstra also founded Tesla.",
        ...
        segment1 = TranscriptSegmentDB(
            video_id=_VIDEO_ID,
            language_code=_LANGUAGE_CODE,
            text="Edsger Dijkstra founded SpaceX.",
            start_time=0.0,
            duration=5.0,
            end_time=5.0,
            sequence_number=0,
        ...
        segment2 = TranscriptSegmentDB(
            video_id=_VIDEO_ID,
            language_code=_LANGUAGE_CODE,
            text="Later Edsger Dijkstra also founded Tesla.",
            start_time=5.0,
            duration=4.0,
            end_time=9.0,
            sequence_number=1,
        ...
        existing_entity = (
            await session.execute(
                select(NamedEntityDB).where(
                    NamedEntityDB.canonical_name == "Edsger Dijkstra",
        ...
        if existing_entity:
            ...
            entity = NamedEntityDB(
                id=entity_id,
                canonical_name="Edsger Dijkstra",
                canonical_name_normalized="edsger dijkstra",
                entity_type="person",
...
class TestGetEntityVideos:
    ...
    async def test_get_entity_videos_200_with_data(
        ...
        for preview in mentions:
            ...
            assert preview["mention_text"] == "Edsger Dijkstra"
...
class TestListEntities:
    ...
    (``"Edsger Dijkstra"``, type ``"person"``).  Tests that require a second entity
    ...
    async def test_list_entities_200_with_data(
        ...
        assert edsger is not None, "Expected 'Edsger Dijkstra' in list response"
        assert edsger["entity_id"] == seed_entity_data["entity_id"]
        assert edsger["entity_type"] == "person"
        assert edsger["status"] == "active"
        assert isinstance(edsger["mention_count"], int)
        assert isinstance(edsger["video_count"], int)
    ...
    async def test_list_entities_filter_by_type(
        ...
        assert elon_org is None, "Did not expect 'Edsger Dijkstra' in type=organization results"
    ...
    async def test_list_entities_search_by_name_case_insensitive(
        ...
        Queries ``search=edsger`` (lowercase) and verifies "Edsger Dijkstra" is returned.
        Also queries ``search=EDSGER`` (uppercase) to confirm case-insensitivity.
        """
        with patch("chronovista.api.deps.youtube_oauth") as mock_oauth:
            mock_oauth.is_authenticated.return_value = True
            response_lower = await async_client.get(
                _list_entities_url(), params={"search": "edsger"}
            )
    ...
    async def test_list_entities_sort_by_name_default(
        ...
        (alphabetically before ``"Edsger Dijkstra"``), queries without ``sort``, verifies
    ...
    async def test_list_entities_empty_result_set(
        ...
        Uses ``type=organization`` combined with ``search=Edsger Dijkstra`` (which is typed
        ...
        with patch("chronovista.api.deps.youtube_oauth") as mock_oauth:
            ...
            response = await async_client.get(
                _list_entities_url(),
                params={"type": "organization", "search": "Edsger Dijkstra"},
            )

        assert response.status_code == 200, response.text
        body = response.json()
        assert body["data"] == [], (
            f"Expected empty data for type=organization+search=Edsger Dijkstra, got: {body['data']}"
        )
        assert body["pagination"]["total"] == 0
        assert body["pagination"]["has_more"] is False
...
class TestGetEntityDetail:
    ...
    async def test_get_entity_detail_200(
        ...
        the seeded NamedEntity for ``"Edsger Dijkstra"``.
        """
        entity_id = seed_entity_data["entity_id"]
        ...
        assert detail["canonical_name"] == "Edsger Dijkstra"
...
async def seed_search_data(
    ...
    async with integration_session_factory() as session:
        ...
        # ---- Active NamedEntity: Barbara Liskov ---------------------------
        existing_active = (
            await session.execute(
                select(NamedEntityDB).where(
        ...
        if not existing_active:
            session.add(
                NamedEntityDB(
                    ...
                    canonical_name_normalized="barbara liskov",
                    entity_type="person",
        ...
        # Attach "Barbara Liskov" to the search video via rule_match so
    ...
    async def test_search_canonical_name_match(
        ...
        entity = next(
            (
                ...
                if item.get("canonical_name") == "Barbara Liskov"
            ),
            None,
        )
...
class TestCreateManualAssociationEndpoint:
    ...
    async def test_create_201_success(
        ...
        assert data["mention_text"] == "Barbara Liskov", (
            f"Expected mention_text='Barbara Liskov', got: {data['mention_text']}"
        )
...
class TestPerformanceAssertions:
    ...
    async def test_nfr_002_entity_videos_loads_under_2000ms(
        ...
        Uses the ``seed_entity_data`` fixture which seeds the "Edsger Dijkstra"
