...
class TestOrphanRepair:
    async def test_moves_the_alias_to_the_recorded_merge_target(self) -> None:
        target = _tag("thomas kellerman", "active")
        source = _tag("tommy", "merged", merged_into_id=target.id)
        alias = _alias("Tommy", source.id, "tommy")
        ...
        assert moved.to_normalized_form == "thomas kellerman"
        assert moved.from_normalized_form == "tommy"
