...
def _make_named_entity_row(entity_id: str | None = None) -> MagicMock:
    ...
    row.canonical_name = "Edsger Dijkstra"
...
class TestCreateEntityAliasConflict:
    ...
    async def test_duplicate_alias_returns_409(self) -> None:
        ...
        async for client in _build_client(mock_session):
            with patch(
                ...
                mock_normalizer.normalize.return_value = "edsger"

                response = await client.post(
                    _alias_url(),
                    json={"alias_name": "Edsger", "alias_type": "nickname"},
                )
    ...
    async def test_duplicate_alias_message_names_existing_and_explains_folding(
        ...
        existing_alias = MagicMock()
        existing_alias.alias_name = "Peru"
        ...
        async for client in _build_client(mock_session):
            with patch(
                ...
                mock_normalizer.normalize.return_value = "peru"
        ...
        assert "Peru" in detail, f"Message must name existing alias: {detail!r}"
