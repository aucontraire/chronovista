...
from src.chronovista.services.seeding.base_seeder import BaseSeeder, SeedResult, ProgressCallback
...
class MockSeeder(BaseSeeder):
    ...
    def __init__(self, data_type: str, dependencies: set = None):
        ...
        self.seed_result = SeedResult(created=1, updated=0, failed=0, duration_seconds=0.1)
...
class TestSeedingOrchestrator:
    ...
    def mock_takeout_data(self):
        ...
        return TakeoutData(takeout_path=Path("/test/takeout"), subscriptions=[], watch_history=[], playlists=[])
    ...
    def test_dependency_resolution_complex_chain(self, orchestrator):
        ...
        execution_order = orchestrator._resolve_dependencies({"type1", "type2", "type3", "type4"})
    ...
    async def test_seed_execution_order(self, orchestrator, mock_session, mock_takeout_data):
    ...
    async def test_seed_with_progress_callback(self, orchestrator, mock_session, mock_takeout_data):
    ...
    async def test_seed_error_handling(self, orchestrator, mock_session, mock_takeout_data):
    ...
    async def test_seed_empty_types_request(self, orchestrator, mock_session, mock_takeout_data):
    ...
    async def test_real_world_dependency_scenario(self, orchestrator, mock_session, mock_takeout_data):
        ...
        for seeder in [channels_seeder, videos_seeder, user_videos_seeder, playlists_seeder]:
            orchestrator.register_seeder(seeder)
        ...
        assert all(seeder.seed_called for seeder in [channels_seeder, videos_seeder, user_videos_seeder, playlists_seeder])
