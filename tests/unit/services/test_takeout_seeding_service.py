...
from chronovista.services.seeding.base_seeder import SeedResult, ProgressCallback
...
class TestTakeoutSeedingService:
    ...
    async def test_seeding_service_initialization(self, seeding_service: TakeoutSeedingService) -> None:
    ...
    async def test_get_available_types(self, seeding_service: TakeoutSeedingService) -> None:
    ...
    async def test_seed_database_complete_workflow(
        ...
        mock_results = {
            "channels": SeedResult(created=2, updated=0, failed=0, duration_seconds=1.0),
            "videos": SeedResult(created=2, updated=0, failed=0, duration_seconds=2.0),
            "user_videos": SeedResult(created=2, updated=0, failed=0, duration_seconds=1.5),
            "playlists": SeedResult(created=1, updated=0, failed=0, duration_seconds=0.5),
        }
        
        with patch.object(seeding_service.orchestrator, 'seed', new_callable=AsyncMock) as mock_seed:
    ...
    async def test_seed_database_with_skip_types(
        ...
        with patch.object(seeding_service.orchestrator, 'seed', new_callable=AsyncMock) as mock_seed:
            ...
            expected_types = {"channels", "videos"}
    ...
    async def test_seed_incremental_workflow(
        ...
        # Mock the orchestrator's seed method
        mock_results = {
            "channels": SeedResult(created=1, updated=1, failed=0, duration_seconds=1.0),
    ...
    def test_service_with_custom_user_id(self, sample_takeout_data: TakeoutData) -> None:
